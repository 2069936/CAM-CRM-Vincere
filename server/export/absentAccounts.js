...
import {
  ...
  isoDay,
  nameKey,
} from '../../src/domain/accountAbsence.js';
...
// The five-way absence split, the reason prose and the existsFrom rule now live
// in src/domain/accountAbsence.js so the client daily report can apply the same
// policy without re-deriving it. Re-exported here because this module's payload
// legend and its tests import them from this path.
export { ABSENCE_REASONS, ABSENCE_REASON_TEXT, EXISTS_FROM_BASIS };

function existsFromOf(account, firstObservedDate) {
  return existsFrom(account?.date_added, firstObservedDate);
}
