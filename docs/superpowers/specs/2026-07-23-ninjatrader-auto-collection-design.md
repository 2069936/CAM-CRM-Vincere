...
- A signed, guided Windows installer downloadable from the CRM that installs
  both the agent and AddOn without requiring developer tools.
...
session. The interface allows an authorized operator to complete guided pairing,
configure the schedule and enabled days, and run a manual capture.

### 4.3 Guided Installer

The CRM provides a universal, digitally signed `Vincere Auto-Collector Setup`
package. The installer runs as a simple wizard and:

1. requests Windows administrator permission;
2. detects the NinjaTrader installation and Documents data directory;
3. verifies the supported NinjaTrader version;
4. asks for a short, one-time pairing code generated by the CRM;
5. installs the NinjaTrader AddOn and Windows Service;
6. configures the default 4:45 p.m. New York schedule;
7. guides the operator through any required NinjaTrader restart;
8. tests AddOn, agent, and CRM connectivity; and
9. confirms the resolved client name and successful installation.

The normal installation does not ask for a CAM login, Supabase credentials,
NinjaTrader username/password, source-code paths, or command-line input.

The collector package is common to all clients. Per-client installers are not
built or signed dynamically; client enrollment happens through the one-time
...
client resolved through its product key. Subsequent ingest requests authenticate
the issued device credential and machine ID, validate the payload structure,
record an immutable ingest batch, store the compressed raw snapshot, and pass
normalized data into the same reconciliation and persistence path used by
manual CSV uploads.
...
The product key remains the authoritative client identity in the CRM. IP
addresses, client names, filenames, and NinjaTrader account names are not used
to select the CRM client. Employees do not need to copy the product key onto the
VPS; a one-time enrollment code safely delegates the pairing.

On first pairing:

1. From the CRM client profile, an authorized CAM or Manager clicks `Set up
   Auto-Collection`.
2. The CRM creates a random, single-use enrollment code linked server-side to
   that client's product key. The code expires after 60 minutes.
3. The operator downloads the common signed installer and runs it on the VPS.
4. The installer asks only for the enrollment code and obtains the Windows
   `MachineGuid` automatically.
5. The CRM consumes the code, resolves the product key to exactly one active
   client, binds the device, and issues a revocable device credential.
6. The agent stores the device credential using Windows protection and shows the
   resolved client name so the operator can confirm the pairing.
...
- An invalid or expired enrollment code is rejected and can be regenerated from
  the client profile.
...
- The product key remains server-side. The revocable device credential is
  protected at rest with Windows DPAPI or Windows Credential Manager and is
  never stored in a plain-text configuration file.
...
### 10.2 Profile setup experience

Each client profile includes an `Auto-Collection` card showing:

- installation state (`Not installed`, `Paired`, `Online`, `Offline`, or
  `Update required`);
- bound VPS and last communication;
- current schedule and installed versions;
- `Download installer`;
- `Generate pairing code`;
- `Test connection` when paired;
- `Replace VPS / reset binding`; and
- concise numbered setup instructions for non-technical employees.

A CAM profile may also list its assigned clients and expose the same setup
action beside each client. The generated pairing code is always client-specific
even when initiated from the CAM overview.

Installer UX must use plain operational language, show progress for every step,
offer a retry action on recoverable failures, and finish with an explicit
`Auto-collection is ready for <client>` confirmation.

### 10.3 Audit Logs integration
...
- **Invalid or revoked device credential:** reject without exposing client
  information and retain the local snapshot until the VPS is paired again.
- **Invalid or expired pairing code:** stop installation before device binding,
  explain how to generate a new code, and make retry possible without reinstalling.
...
- One-time enrollment, expiration, replay prevention, and device-token
  revocation tests.
...
2. Complete the agent, signed guided installer, server endpoint, storage, and
   Manager/client profile status surfaces.
...
- A non-technical employee can install and pair the collector from a client
  profile using a one-time code, without command-line work or CRM credentials.
